...
###### Helpers #####################
source("source/name_outcome.R")
source("source/name_outcome_vaccs.R")
source("source/not_in.R")
source("source/select_new.R")
source("source/select_vaccines.R")
source("source/rob_tools.R")
source("source/helpers.R")
source("source/forest_invisible.R")
source("source/rob_blobbogram.R")
source("source/create_table.R")
source("source/create_table_vaccs.R")
source("source/help_data.R")
source("source/hospitalized_comparisons.R")
source("source/order_comparisons.R")
source("source/pairwise_old.R")

#########################################################################


############# Analysis functions for treatments #########################
source("source/forest_Score7_D14_D28.R")
source("source/forest_Score7_D60.R")
...
source("source/forest_Hospitalized.R")
source("source/models_treats.R")

###################################################################################################



############# Analysis functions for vaccines #########################


source("source/vaccs_adverse events.R")
source("source/vaccs_serious.R")
source("source/vaccs_mortality.R")
source("source/vaccs_systemic.R")
source("source/vaccs_local.R")
source("source/vaccs_Effcacy_Conf+Symp.R")
source("source/vaccs_Efficacy_Conf.R")
source("source/vaccs_Efficacy_severe or critical.R")
...
codes_treats=readxl::read_xlsx("codes.xlsx")
codes_vaccs=readxl::read_xlsx("codes_vaccs.xlsx")

#out_comp=comparisons_outpatients$comp.num


data = read.csv("data_treats.csv",na = c("*", "NA", "", " ", " *"))
                
  
data = subset(data,  data$Study_design == "RCT" & Trial_ID != 185 & Trial_ID != 280 & Trial_ID != 284 
              & Trial_ID != 247 & Trial_ID != 249  & Trial_ID != 298  #& Trial_ID != 502 
              & Trial_ID != 155 & Trial_ID != 451)
                
data$Research_question=ifelse(data$Research_question=="Mild outpatients","Outpatients",data$Research_question)
    
############################################################################################ 
    

################### Vaccines #########################################
data_vacc=read.csv("data_vaccs.csv",na=c("*","NA",""," "," *"))

#data_vacc=subset(data_vacc,Trial_ID==182)

data_vacc = subset(data_vacc,Trial_ID != 176)

#data_vacc=read.csv("data_vacc_review.csv",na=c("*","NA",""," "," *"))

#data_vacc = subset(data_vacc,Trial_ID != 70)
#######################################################################

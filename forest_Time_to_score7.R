...
#data=read.csv("C:\\Users\\Theodoros Evrenoglou\\Desktop\\Covid-19 NMA\\Database\\October\\22-10-2020\\rct__updated_database_22_10_2020_new.csv", na=c("*","NA"))
forest_Time_to_Score7=function(data,comp.num,sliderRob=0,sliderDose=0,high_RoB=FALSE,keep_only="All population",model="Random-effects",
  ...
  data=data[,c("Trial_ID","ID_stats","name_study","First_author","Year","Study_design","Funding","Conflict_of_interest_category",
               ...
               "Time_to_score_7_and_above_HR","Time_to_score_7_and_above_LCI","Time_to_score_7_and_above_UCI",
               "ROB_1_randomization","ROB2_Time_score7","ROB_2_Deviations_from_intervention","ROB_3_Time_to_SCORE_7_and_above_WHO",
               "ROB_4_SCORE_7_and_above_WHO","ROB_5_Time_to_SCORE_7_and_above_WHO","ROB_6_Time_to_SCORE_7_and_above_WHO"
 ...
 # data$NEG_conv_denominator=ifelse(is.na(data$NEG_conv_denominator),data$n_Randomized,data$NEG_conv_denominator)
  ...
  pair_data$Time_to_score_7_and_above_HR=as.numeric(pair_data$Time_to_score_7_and_above_HR)
  pair_data$Time_to_score_7_and_above_LCI=as.numeric(pair_data$Time_to_score_7_and_above_LCI)
  pair_data$Time_to_score_7_and_above_UCI=as.numeric(pair_data$Time_to_score_7_and_above_UCI)
 ...
 # analysis_data_11b$N1=as.numeric(levels(analysis_data_11b$N1))[analysis_data_11b$N1]
#  analysis_data_11b$N2=as.numeric(levels(analysis_data_11b$N2))[analysis_data_11b$N2]
 # analysis_data_11b$comp=as.numeric(levels(analysis_data_11b$comp))[analysis_data_11b$comp]
  #analysis_data_11b$LCI=as.numeric(levels(analysis_data_11b$LCI))[analysis_data_11b$LCI]
  #analysis_data_11b$UCI=as.numeric(levels(analysis_data_11b$UCI))[analysis_data_11b$UCI]
  #analysis_data_11b$HR=as.numeric(levels(analysis_data_11b$HR))[analysis_data_11b$HR]
  ...
  #analysis_data_11b$sev=as.numeric(levels(analysis_data_11b$sev))[analysis_data_11b$sev]
  #analysis_data_11b$subgroup=as.numeric(levels(analysis_data_11b$subgroup))[analysis_data_11b$subgroup]
  ...
  if(keep_only=="Mild population"){
    data_sev<- analysis_data_11b %>%
      filter(analysis_data_11b$sev==1)
    analysis_data_11b=data_sev
  }
  
  if(keep_only=="Mixed population"){
    
    data_sev<-analysis_data_11b %>%
      filter(analysis_data_11b$sev %in% c(2:11))
  ...
  if(keep_only=="Critical population"){
    ...
    data_sev<-analysis_data_11b %>%
      filter(analysis_data_11b$sev==12)
  ...
  comp.num=comp.num
  #file_name="comp.num_11b_pharma_rct.jpeg"
 #file_name=paste(comp.num,"_11b_pharma_rct.jpeg",sep="")
 #jpeg(file=file_name,width=2431,height=1346,res=200)
  #meta-analysis#
  #rm(result_11b)
  #rm(result_11b.3)
  #analysis_data_11b=subset(analysis_data_11b_all,comp==comp.num & !is.na(logHR) & !is.na(se))
  ...
  if(nrow(analysis_data_11b)>0){
  
  ################################################################
  ################################################################
  exclude_rob<- analysis_data_11b %>%
    filter(analysis_data_11b$Overall=="High")
  
  w=setdiff(analysis_data_11b$First_author,exclude_rob$First_author)
  
  if(high_RoB==TRUE){
    analysis_data_11b<-subset(analysis_data_11b,analysis_data_11b$First_author %in% w)}
  
  if(high_RoB==FALSE){
    analysis_data_11b=analysis_data_11b}
  
  analysis_data_11b=analysis_data_11b[order(analysis_data_11b$sev,decreasing=TRUE),]
  ilab=cbind(F_U_days=as.character(analysis_data_11b$F_U_days),Intervention1=as.character(analysis_data_11b$Intervention1),
             Intervention2=as.character(analysis_data_11b$Intervention2),
             counts1=analysis_data_11b$N1,counts2=analysis_data_11b$N2)
  ilab=as.data.frame(ilab)
  
  
  ########################################################################
  analysis_data_11b_0<-subset(analysis_data_11b,analysis_data_11b$comp==comp.num & analysis_data_11b$subgroup==0)
  analysis_data_11b_1<-subset(analysis_data_11b,analysis_data_11b$comp==comp.num & analysis_data_11b$subgroup==1)
  analysis_data_11b_2<-subset(analysis_data_11b,analysis_data_11b$comp==comp.num & analysis_data_11b$subgroup==2)
  ##########################################################################
  
  result_11b=rma(measure="GEN",yi=analysis_data_11b$logHR,sei=analysis_data_11b$se,
                 slab=paste(analysis_data_11b$First_author,analysis_data_11b$Year,sep=", "),
                 subset=(analysis_data_11b$comp==comp.num))
  
  result_111b=metagen(TE=analysis_data_11b$logHR,seTE=analysis_data_11b$se,byvar = analysis_data_11b$subgroup)
  
  if(model=="Fixed-effects"){
    result_11b=update(result_11b,tau2=0)
  ...
  ks1=0
  ks2=0
  if (result_11b$k>1 & nrow(analysis_data_11b_0)>0){
    result_11b.0=rma(measure="GEN",yi=analysis_data_11b_0$logHR,sei=analysis_data_11b_0$se,
                     slab=paste(analysis_data_11b_0$First_author,analysis_data_11b_0$Year,sep=", "))
    
    if(model=="Fixed-effects"){
      result_11b.0=update(result_11b.0,tau2=0)
      
    }
    
    ks0=result_11b.0$k
  }
  if (result_11b$k>1 & nrow(analysis_data_11b_1)>0){
    result_11b.1=rma(measure="GEN",yi=analysis_data_11b_1$logHR,sei=analysis_data_11b_1$se,
                     slab=paste(analysis_data_11b_1$First_author,analysis_data_11b_1$Year,sep=", "))
    
    if(model=="Fixed-effects"){
      result_11b.1=update(result_11b.1,tau2=0)
      
    }
    
    ks1=result_11b.1$k
  }
  if (result_11b$k>1 & nrow(analysis_data_11b_2)>0){
    result_11b.2=rma(measure="GEN",yi=analysis_data_11b_2$logHR,sei=analysis_data_11b_2$se,
                     slab=paste(analysis_data_11b_2$First_author,analysis_data_11b_2$Year,sep=", "))
    
    
    if(model=="Fixed-effects"){
      result_11b.2=update(result_11b.2,tau2=0)
  ...
  result_11b.3=result_11b
  if (result_11b$k==1){
    allrows=1
  }
  if (result_11b$k>1){
    if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
      allrows=c(1:result_11b$k)    
    }
  ...
  metafor::forest(result_11b,atransf=exp,showweights=TRUE, xlab="Hazard Ratio",header=c("Study                                   Study Duration         Intervention 1               Intervention 2          N1       N2                                                                 Risk of Bias
                                                    days                                                                                                                                                                A       B       C       D       E       Overall"),cex=0.73,mlab="",
                  col="white",border="white",xlim=c(-28,20),alim=c(floor(result_11b$ci.lb)-2,ceiling(result_11b$ci.ub))+1,steps=4,
  ...
  addpoly.rma(result_11b.3,row=-1,mlab="",col="darkblue",border="darkblue",atransf=exp,cex=0.9)
  if (result_11b$k>1){
    if (ks0>0 & ks1>0 & ks2==0){
      if(sub=="Severity"){
      addpoly.rma(result_11b.1,row=1,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_11b.0,row=ks1+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      if(sub=="Conflicts of interest"){
        addpoly.rma(result_11b.1,row=1,mlab="                    Studies with conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_11b.0,row=ks1+2,mlab="                    Studies without conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9)
        
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_11b.1,row=1,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_11b.0,row=ks1+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9)
        
      }
      
      if(sub=="Location"){
        addpoly.rma(result_11b.1,row=1,mlab="                    National studies",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_11b.0,row=ks1+2,mlab="                    Multinational studies",col="blue",border="blue",atransf=exp,cex=0.9)
    ...
    if (ks0>0 & ks1==0 & ks2>0){
      if(sub=="Severity"){
      addpoly.rma(result_11b.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_11b.0,row=ks2+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_11b.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_11b.0,row=ks2+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
    ...
    if (ks0==0 & ks1>0 & ks2>0){
      if(sub=="Severity"){
      addpoly.rma(result_11b.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_11b.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_11b.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_11b.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
    }
    ...
    if (ks0>0 & ks1>0 & ks2>0){
      ...
      addpoly.rma(result_11b.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_11b.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_11b.0,row=ks2+4+ks1,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_11b.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_11b.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_11b.0,row=ks2+4+ks1,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
    }        
  }
  ...
  text(-6,3.5+max(allrows),"Pharmacological treatments",
       ...
       cex=1,
       col = "darkblue")
  text(-9,3+max(allrows),"Time to WHO progression score level 7 or above",
       pos=4,
       cex=1)
  text(-13,2.7+max(allrows),"[mechanical ventilation +/- additional organ support (ECMO, vasopressors or dialysis) OR death]",
       pos=4, cex=0.9)
  
  #get rob data#
  rob_data_11b=cbind(Trial_ID=data$Trial_ID,A=as.character(data$ROB_1_randomization),
                     B=ifelse(is.na(data$ROB2_Time_score7),as.character(data$ROB_2_Deviations_from_intervention),as.character(data$ROB2_Time_score7)),
                     C=as.character(data$ROB_3_Time_to_SCORE_7_and_above_WHO),D=as.character(data$ROB_4_SCORE_7_and_above_WHO),E=as.character(data$ROB_5_Time_to_SCORE_7_and_above_WHO),
                     Overall=as.character(data$ROB_6_Time_to_SCORE_7_and_above_WHO))
  rob_data_11b=as.data.frame(rob_data_11b)
  ncomp=length(analysis_data_11b$Trial_ID)
  A=rep("NA",ncomp)
  B=rep("NA",ncomp)
  C=rep("NA",ncomp)
  D=rep("NA",ncomp)
  ...
  nst=length(rob_data_11b$Trial_ID)
  for(i in 1:ncomp){
    for(j in 1:nst){
      if(rob_data_11b$Trial_ID[j]==analysis_data_11b$Trial_ID[i]){
        A[i]=as.character(rob_data_11b$A[j])
        B[i]=as.character(rob_data_11b$B[j])
        C[i]=as.character(rob_data_11b$C[j])
        D[i]=as.character(rob_data_11b$D[j])
        E[i]=as.character(rob_data_11b$E[j])
        Overall[i]=as.character(rob_data_11b$Overall[j])
      }   
    }
  }
  A=trimws(A)
  ...
  analysis_data_11b$A=A
  analysis_data_11b$B=B
  analysis_data_11b$C=C
  analysis_data_11b$D=D
  analysis_data_11b$E=E
  analysis_data_11b$Overall=Overall
  analysis_data_11b$A[analysis_data_11b$A=="High"]="red"
  analysis_data_11b$A[analysis_data_11b$A=="Low"]="chartreuse3"
  analysis_data_11b$A[analysis_data_11b$A=="Some concerns"]="gold"
  analysis_data_11b$B[analysis_data_11b$B=="High"]="red"
  analysis_data_11b$B[analysis_data_11b$B=="Low"]="chartreuse3"
  analysis_data_11b$B[analysis_data_11b$B=="Some concerns"]="gold"
  analysis_data_11b$C[analysis_data_11b$C=="High"]="red"
  analysis_data_11b$C[analysis_data_11b$C=="Low"]="chartreuse3"
  analysis_data_11b$C[analysis_data_11b$C=="Some concerns"]="gold"
  analysis_data_11b$D[analysis_data_11b$D=="High"]="red"
  analysis_data_11b$D[analysis_data_11b$D=="Low"]="chartreuse3"
  analysis_data_11b$D[analysis_data_11b$D=="Some concerns"]="gold"
  analysis_data_11b$E[analysis_data_11b$E=="High"]="red"
  analysis_data_11b$E[analysis_data_11b$E=="Low"]="chartreuse3"
  analysis_data_11b$E[analysis_data_11b$E=="Some concerns"]="gold"
  analysis_data_11b$Overall[analysis_data_11b$Overall=="High"]="red"
  analysis_data_11b$Overall[analysis_data_11b$Overall=="Low"]="chartreuse3"
  analysis_data_11b$Overall[analysis_data_11b$Overall=="Some concerns"]="gold"
  #analysis_rob_11b=subset(analysis_data_11b,comp==comp.num & !is.na(logHR) & !is.na(se))
  
  
  analysis_rob_11b= analysis_data_11b %>%
    filter(analysis_data_11b$comp==comp.num)
  
  analysis_rob_11b=analysis_rob_11b[complete.cases(analysis_rob_11b$UCI), ]
  
  
  # if(den=="Randomized"){
    
  if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
    for(i in 1:result_11b$k){
      legend(x=3.2,y=0.41+i+sliderRob,legend=c("", "","","","",""),
             col=c(analysis_rob_11b$A[i],analysis_rob_11b$B[i],analysis_rob_11b$C[i],analysis_rob_11b$D[i],analysis_rob_11b$E[i],analysis_rob_11b$Overall[i]), 
             cex=0.75,inset=0.1,bg="white",
             box.lty=0,title = "",fill = c(analysis_rob_11b$A[i],analysis_rob_11b$B[i],analysis_rob_11b$C[i],analysis_rob_11b$D[i],analysis_rob_11b$E[i],analysis_rob_11b$Overall[i]),pt.cex = 5
             ,x.intersp=0.5,
             horiz = TRUE,
             border = c(analysis_rob_11b$A[i],analysis_rob_11b$B[i],analysis_rob_11b$C[i],analysis_rob_11b$D[i],analysis_rob_11b$E[i],analysis_rob_11b$Overall[i]),
  ...
  if(comp.num==comp.num){
    if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
      for(i in 1:result_11b$k){
        text(-16.5, -0.3+i+sliderDose, pos=4, analysis_rob_11b$dose[i],cex=0.7)
      }
    }
    if (ks0>0 & ks1>0 & ks2==0){
      for(i in 1:ks1){
        text(-16.5, -0.3+i+1+sliderDose, pos=4, analysis_rob_11b$dose[i],cex=0.7)
      }
      for(i in (ks1+1):(ks0+ks1)){
        text(-16.5, -0.3+i+2+sliderDose, pos=4, analysis_rob_11b$dose[i],cex=0.7)
      }
    }
    if (ks0>0 & ks1==0 & ks2>0){
      for(i in 1:ks2){
    ...
    if (ks0>0 & ks1>0 & ks2>0){
      ...
      for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
        text(-16.5, -0.3+i+4+sliderDose, pos=4, analysis_rob_11b$dose[i],cex=0.7)
      }
    }
  }
  
  
  count=sum(str_count(analysis_rob_11b$dose, "\\*"),na.rm = TRUE)
  ...
  if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
    for(i in 1:result_11b$k){
      ...
      text(-28, 0.5+i, pos=4, analysis_rob_11b$severity[i])
      }
    }
  }
  if (ks0>0 & ks1>0 & ks2==0){
    for(i in 1:ks1){
      if(hide_severity=="FALSE"){
      text(-28, 0.5+i+1, pos=4, analysis_rob_11b$severity[i])
      }
    }
    for(i in (ks1+1):(ks0+ks1)){
      if(hide_severity=="FALSE"){
      text(-28, 0.5+i+2, pos=4, analysis_rob_11b$severity[i])
      }
    }
  }
  if (ks0>0 & ks1==0 & ks2>0){
  ...
  if (ks0>0 & ks1>0 & ks2>0){
    ...
    for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
      if(hide_severity=="FALSE"){
      text(-28, 0.5+i+4, pos=4, analysis_rob_11b$severity[i])
      }
    }
  }
  ...
  if(result_11b$k>1){
    if(length(unique(analysis_data_11b$subgroup))==1){
    text(-28, 0.2, pos=4, cex=0.9, bquote(paste("Heterogeneity: Q = ",
                                                .(formatC(result_11b$QE, digits=2, format="f")),
                                                ", p = ", .(formatC(result_11b$QEp, digits=2, format="f")), "; ", I^2, " = ",
                                                .(formatC(result_11b$I2, digits=1, format="f")), "%; ",tau^2," = ",
                                                .(formatC(result_11b$tau2, digits=2, format="f")))))  
    ...
    if(length(unique(analysis_data_11b$subgroup))>1){
      ...
      text(-28, 0.2, pos=4, cex=0.9, bquote(paste("Heterogeneity: Q = ",
                                                  ...
                                                  ", p = ", .(formatC(result_11b$QEp, digits=2, format="f")), "; ", I^2, " = ",
                                                  .(formatC(result_11b$I2, digits=1, format="f")), "%; ",tau^2," = ",
                                                  .(formatC(result_11b$tau2, digits=2, format="f")), "; ", "Test for subgroup differences: Q = ",
                                                  .(formatC(result_111b$Q.b.random, digits=2, format="f")),"; ", "p = ",
                                                  .(formatC(result_111b$pval.Q.b.random, digits=2, format="f"))
  ...
  else if (nrow(analysis_data_11b)==0){

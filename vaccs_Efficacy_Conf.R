...
forest_confirmed=function(data,
                       type,slider=0,
                       subgroup=T,
                       overall=F,
...
data$F_U_months=ifelse(is.na(data$Confirmed_COVID_Timepoint),as.character(data$Efficacy_median_followup),data$Confirmed_COVID_Timepoint)
data$F_U_days=data$F_U_months
data$Start_F_U_days=as.character(data$Conf_sympt_time_zero)
data$n_Randomized=as.numeric(data$Confirmed_COVID_denominator)
...
data$status=ifelse(is.na(data$Any_Vaccine_efficacy_LCI),"Incomplete data",data$status)

data$status=ifelse(is.na(data$Any_Vaccine_efficacy_UCI),"Incomplete data",data$status)
...
rct_data$Confirmed=as.numeric(rct_data$Confirmed_COVID)
pair_data=pairwise_old(studlab=Trial_ID,treat=Treat_Name,event=Confirmed_COVID,n=Confirmed_COVID_denominator,
...
if(type %!in% c("Inactivated virus","Protein subunit","Non replicating viral vector","RNA based vaccine","Virus-Like particle")){
  pair_data$Conf_sympt1=as.numeric(pair_data$Any_Vaccine_efficacy_perc1)
  pair_data$Conf_sympt_CI_Lower1=as.numeric(pair_data$Any_Vaccine_efficacy_LCI1)
  pair_data$Conf_sympt_CI_Upper1=as.numeric(pair_data$Any_Vaccine_efficacy_UCI1)
  
}

if(type %in% c("Inactivated virus","Protein subunit","Non replicating viral vector","RNA based vaccine","Virus-Like particle")){
  pair_data$Conf_sympt1=as.numeric(pair_data$Any_Vaccine_efficacy_perc)
  pair_data$Conf_sympt_CI_Lower1=as.numeric(pair_data$Any_Vaccine_efficacy_LCI)
  pair_data$Conf_sympt_CI_Upper1=as.numeric(pair_data$Any_Vaccine_efficacy_UCI)
...
if(type %in% c("Inactivated virus","Protein subunit","Non replicating viral vector","RNA based vaccine","Virus-Like particle")){
  ...
  new_data$Any_Vaccine_efficacy_UCI1=new_data$Any_Vaccine_efficacy_UCI
  new_data$Any_Vaccine_efficacy_LCI1=new_data$Any_Vaccine_efficacy_LCI
  new_data$Any_Vaccine_efficacy_perc1=new_data$Any_Vaccine_efficacy_perc
  
}

if(type %in% c("RNA based vaccine")){
...
analysis_data_09=cbind.data.frame(Trial_ID=new_data$Trial_ID,First_author=new_data$First_author,
                                  ...
                                  percentage=new_data$Any_Vaccine_efficacy_perc1,LCI=new_data$Any_Vaccine_efficacy_LCI1,
                                  UCI=new_data$Any_Vaccine_efficacy_UCI1,
                                  
                                  sev=new_data$sev,
                                  #subgroup=new_data$subgroup,
                                  Year=new_data$Year,
                                  # dose=as.character(new_data$Treat_Dose_desc1),
                                  A=as.character(new_data$ROB_1_randomization),
                                  B=as.character(new_data$ROB2_Confirmed_COVID),
                                  C=as.character(new_data$ROB3_ConfirmedCOVID),
                                  D=as.character(new_data$ROB4_Confirmed_COVID),
                                  E=as.character(new_data$ROB5_Confirmed_COVID),
                                  Overall=as.character(new_data$ROB6_Confirmed_COVID),
...
analysis_data_09$rate=#1-
  analysis_data_09$percentage/100

analysis_data_09$rate1=analysis_data_09$rate
#ifelse(analysis_data_09$rate==0,0.05,analysis_data_09$rate)

analysis_data_09$UCI_rate=(analysis_data_09$UCI/100)
#1-(analysis_data_09$LCI/100)

analysis_data_09$LCI_rate=(analysis_data_09$LCI/100)
#1-(analysis_data_09$UCI/100)


analysis_data_09$LCI_rate1=analysis_data_09$LCI_rate
#ifelse(analysis_data_09$LCI_rate==0,analysis_data_09$UCI_rate-0.05,analysis_data_09$LCI_rate)

analysis_data_09$rate=analysis_data_09$rate1#log(analysis_data_09$rate1)
...
analysis_data_09$LCI_table=analysis_data_09$LCI
analysis_data_09$UCI_table=analysis_data_09$UCI

analysis_data_09$LCI_table=ifelse(analysis_data_09$LCI_table<0,0,analysis_data_09$LCI_table)
analysis_data_09$UCI_table=ifelse(analysis_data_09$UCI_table<1,1,analysis_data_09$UCI_table)

Ef=paste(format(round(analysis_data_09$percentage,digits = 2),
                nsmall=2),"% ",sep="")
Ef1=paste("[",format(round(analysis_data_09$LCI_table,digits = 2),
                     nsmall=2),"%",sep = "") 
Ef2=paste(",",format(round(analysis_data_09$UCI_table,digits = 2),
                     nsmall=2),"%","]",sep="") 
Ef_final=paste(Ef,Ef1,Ef2,sep="")
analysis_data_09$Ef_final=Ef_final
...
# header11=c("Study\n(Developer)","Starting\nday","Median follow-up\nmonths","Intervention 1","Intervention 2","r1/N1","r2/N2","Risk of bias","Vaccine Efficacy [95% CI]","Rate ratio [95% CI]")
...
analysis_data_09$LCI_rate=ifelse(analysis_data_09$LCI_rate<0,0,analysis_data_09$LCI_rate)
...
for(i in 1:max(u)){
  ...
  diamond_row[[i]]=min(yy[[i]]$rows)-0.5
  ...
  if(subgroup==T){
    ...
    text(x=-2.1,y=yy[[i]]$rows,w[[i]],pos = 4,cex=0.8)
    if(subgroup==F){
...
text(-2.9,3.5+max(rows),"SARS-CoV-2 infection after complete vaccination",
     pos=4,
     cex=1)


par(font=1)
text(-2.4,3+max(rows),paste("Type:",type,sep=" "),
     pos=4,
     cex=1)
...
if(type=="Non replicating viral vector"){
  ...
  text(-3, -0.6, "MenACWY: Quadrivalent meningococcal conjugate vaccine", pos=4, cex=0.75)

...
forest_Time_to_neg_conv=function(data,comp.num,sliderRob=0,sliderDose=0,high_RoB=FALSE,keep_only="All population",model="Random-effects",
  ...
  data=data[,c("Trial_ID","ID_stats","name_study","First_author","Year","Study_design","Funding","Conflict_of_interest_category",
               ...
               "ROB_4__Viral_Neg_Conv_events","ROB_5_Time_to_NEG_conv","ROB_6_Time_to_NEG_conv"
 ...
 # data$Treat_desc_summary[data$Treat_desc_summary=="at prescribers' discretion "]="-"
  ...
  #analysis_data_13$N1=as.numeric(levels(analysis_data_13$N1))[analysis_data_13$N1]
  #analysis_data_13$N2=as.numeric(levels(analysis_data_13$N2))[analysis_data_13$N2]
  #analysis_data_13$comp=as.numeric(levels(analysis_data_13$comp))[analysis_data_13$comp]
  #analysis_data_13$LCI=as.numeric(levels(analysis_data_13$LCI))[analysis_data_13$LCI]
  #analysis_data_13$UCI=as.numeric(levels(analysis_data_13$UCI))[analysis_data_13$UCI]
  
  #analysis_data_13$HR=as.numeric(levels(analysis_data_13$HR))[analysis_data_13$HR]
  analysis_data_13$logHR=log(analysis_data_13$HR)
  analysis_data_13$se=(log(analysis_data_13$UCI)-log(analysis_data_13$LCI))/3.92
  
  #analysis_data_13$sev=as.numeric(levels(analysis_data_13$sev))[analysis_data_13$sev]
  #analysis_data_13$subgroup=as.numeric(levels(analysis_data_13$subgroup))[analysis_data_13$subgroup]
  ...
  if(keep_only=="Mixed population"){
    ...
    data_sev<-analysis_data_13 %>%
      filter(analysis_data_13$sev %in% c(2:11))
  ...
  if(keep_only=="Critical population"){
    ...
    data_sev<-analysis_data_13 %>%
      filter(analysis_data_13$sev==12)
  ...
  comp.num=comp.num
  #file_name="comp.num_13_pharma_rct.jpeg"
  #file_name=paste(comp.num,"_13_pharma_rct.jpeg",sep="")
  #jpeg(file=file_name,width=2431,height=1346,res=200)
  #meta-analysis#
 #rm(result_13)
 #rm(result_13.3)
  #analysis_data_13=subset(analysis_data_13_all,comp==comp.num & !is.na(logHR) & !is.na(se))
  ...
  ilab=cbind(F_U_days=as.character(analysis_data_13$F_U_days),Intervention1=as.character(analysis_data_13$Intervention1),
             ...
             counts1=analysis_data_13$N1,counts2=analysis_data_13$N2)
  ...
  result_13=rma(measure="GEN",yi=analysis_data_13$logHR,sei=analysis_data_13$se,
                ...
                subset=(analysis_data_13$comp==comp.num))
  
  result_131=metagen(TE=analysis_data_13$logHR,seTE=analysis_data_13$se,byvar = analysis_data_13$subgroup)
  
  if(model=="Fixed-effects"){
    result_13=update(result_13,tau2=0)
  ...
  if (result_13$k>1 & nrow(analysis_data_13_0)>0){
    result_13.0=rma(measure="GEN",yi=analysis_data_13_0$logHR,sei=analysis_data_13_0$se,
                    slab=paste(analysis_data_13_0$First_author,analysis_data_13_0$Year,sep=", "))
    
    if(model=="Fixed-effects"){
      result_13.0=update(result_13.0,tau2=0)
  ...
  if (result_13$k>1 & nrow(analysis_data_13_1)>0){
    result_13.1=rma(measure="GEN",yi=analysis_data_13_1$logHR,sei=analysis_data_13_1$se,
                    slab=paste(analysis_data_13_1$First_author,analysis_data_13_1$Year,sep=", "))
    
    if(model=="Fixed-effects"){
      result_13.1=update(result_13.1,tau2=0)
  ...
  if (result_13$k>1 & nrow(analysis_data_13_2)>0){
    result_13.2=rma(measure="GEN",yi=analysis_data_13_2$logHR,sei=analysis_data_13_2$se,
                    slab=paste(analysis_data_13_2$First_author,analysis_data_13_2$Year,sep=", "))
    
    if(model=="Fixed-effects"){
      result_13.2=update(result_13.2,tau2=0)
  ...
  metafor::forest(result_13,atransf=exp,showweights=TRUE, xlab="Hazard Ratio",header=c("Study                                   Study Duration         Intervention 1               Intervention 2          N1       N2                                                                 Risk of Bias
                                                    days                                                                                                                                                                A       B       C       D       E       Overall"),cex=0.73,mlab="",
                  col="white",border="white",xlim=c(-28,20),alim=c(floor(result_13$ci.lb)-2,ceiling(result_13$ci.ub))+1,steps=4,
  ...
  addpoly.rma(result_13.3,row=-1,mlab="",col="darkblue",border="darkblue",atransf=exp,cex=0.9)
  if (result_13$k>1){
    if (ks0>0 & ks1>0 & ks2==0){
      if(sub=="Severity"){
      addpoly.rma(result_13.1,row=1,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_13.0,row=ks1+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      if(sub=="Conflicts of interest"){
        addpoly.rma(result_13.1,row=1,mlab="                    Studies with conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_13.0,row=ks1+2,mlab="                    Studies without conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      if(sub=="Funding"){
        addpoly.rma(result_13.1,row=1,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_13.0,row=ks1+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      if(sub=="Location"){
        addpoly.rma(result_13.1,row=1,mlab="                    National studies",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_13.0,row=ks1+2,mlab="                    Multinational studies",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      
    }
    if (ks0>0 & ks1==0 & ks2>0){
      if(sub=="Severity"){
      addpoly.rma(result_13.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_13.0,row=ks2+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      if(sub=="Funding"){
        addpoly.rma(result_13.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_13.0,row=ks2+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
    }
    if (ks0==0 & ks1>0 & ks2>0){
      if(sub=="Severity"){
      addpoly.rma(result_13.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_13.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      if(sub=="Funding"){
        addpoly.rma(result_13.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_13.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9)
    ...
    if (ks0>0 & ks1>0 & ks2>0){
      ...
      addpoly.rma(result_13.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_13.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_13.0,row=ks2+4+ks1,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      if(sub=="Funding"){
        addpoly.rma(result_13.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_13.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_13.0,row=ks2+4+ks1,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
    }        
  }
  ...
  text(-6,3.5+max(allrows),"Pharmacological treatments",
       ...
       cex=1,
       col = "darkblue")
  text(-6,3+max(allrows),"Time to negative conversion",
       pos=4,
       cex=1)
  
  #get rob data#
  rob_data_13=cbind(Trial_ID=data$Trial_ID,A=as.character(data$ROB_1_randomization),
                    B=ifelse(is.na(data$ROB2_Time_neg_conv),as.character(data$ROB_2_Deviations_from_intervention),as.character(data$ROB2_Time_neg_conv)),
                    C=as.character(data$ROB_3_Time_to_NEG_conv),D=as.character(data$ROB_4__Viral_Neg_Conv_events),E=as.character(data$ROB_5_Time_to_NEG_conv),
                    Overall=as.character(data$ROB_6_Time_to_NEG_conv))
  rob_data_13=as.data.frame(rob_data_13)
  ncomp=length(analysis_data_13$Trial_ID)
  A=rep("NA",ncomp)
  B=rep("NA",ncomp)
  C=rep("NA",ncomp)
  D=rep("NA",ncomp)
  ...
  nst=length(rob_data_13$Trial_ID)
  for(i in 1:ncomp){
    for(j in 1:nst){
      if(rob_data_13$Trial_ID[j]==analysis_data_13$Trial_ID[i]){
        A[i]=as.character(rob_data_13$A[j])
        B[i]=as.character(rob_data_13$B[j])
        C[i]=as.character(rob_data_13$C[j])
        D[i]=as.character(rob_data_13$D[j])
        E[i]=as.character(rob_data_13$E[j])
        Overall[i]=as.character(rob_data_13$Overall[j])
      }   
    }
  }
  A=trimws(A)
  ...
  analysis_data_13$A=A
  analysis_data_13$B=B
  analysis_data_13$C=C
  analysis_data_13$D=D
  analysis_data_13$E=E
  analysis_data_13$Overall=Overall
  analysis_data_13$A[analysis_data_13$A=="High"]="red"
  analysis_data_13$A[analysis_data_13$A=="Low"]="chartreuse3"
  analysis_data_13$A[analysis_data_13$A=="Some concerns"]="gold"
  analysis_data_13$B[analysis_data_13$B=="High"]="red"
  analysis_data_13$B[analysis_data_13$B=="Low"]="chartreuse3"
  analysis_data_13$B[analysis_data_13$B=="Some concerns"]="gold"
  analysis_data_13$C[analysis_data_13$C=="High"]="red"
  analysis_data_13$C[analysis_data_13$C=="Low"]="chartreuse3"
  analysis_data_13$C[analysis_data_13$C=="Some concerns"]="gold"
  analysis_data_13$D[analysis_data_13$D=="High"]="red"
  analysis_data_13$D[analysis_data_13$D=="Low"]="chartreuse3"
  analysis_data_13$D[analysis_data_13$D=="Some concerns"]="gold"
  analysis_data_13$E[analysis_data_13$E=="High"]="red"
  analysis_data_13$E[analysis_data_13$E=="Low"]="chartreuse3"
  analysis_data_13$E[analysis_data_13$E=="Some concerns"]="gold"
  analysis_data_13$Overall[analysis_data_13$Overall=="High"]="red"
  analysis_data_13$Overall[analysis_data_13$Overall=="Low"]="chartreuse3"
  analysis_data_13$Overall[analysis_data_13$Overall=="Some concerns"]="gold"
  #analysis_rob_13=subset(analysis_data_13,comp==comp.num & !is.na(logHR) & !is.na(se))
  
  analysis_rob_13= analysis_data_13 %>%
    filter(analysis_data_13$comp==comp.num)
  
  analysis_rob_13=analysis_rob_13[complete.cases(analysis_rob_13$UCI), ]
  ...
  if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
    for(i in 1:result_13$k){
      legend(x=3.2,y=0.41+i+sliderRob,legend=c("", "","","","",""),
             col=c(analysis_rob_13$A[i],analysis_rob_13$B[i],analysis_rob_13$C[i],analysis_rob_13$D[i],analysis_rob_13$E[i],analysis_rob_13$Overall[i]), 
             cex=0.75,inset=0.1,bg="white",
             box.lty=0,title = "",fill = c(analysis_rob_13$A[i],analysis_rob_13$B[i],analysis_rob_13$C[i],analysis_rob_13$D[i],analysis_rob_13$E[i],analysis_rob_13$Overall[i]),pt.cex = 5
             ,x.intersp=0.5,
             horiz = TRUE,
             border = c(analysis_rob_13$A[i],analysis_rob_13$B[i],analysis_rob_13$C[i],analysis_rob_13$D[i],analysis_rob_13$E[i],analysis_rob_13$Overall[i]),
  ...
  if(comp.num==comp.num){
    if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
      for(i in 1:result_13$k){
        text(-16.5, -0.3+i+sliderDose, pos=4, analysis_rob_13$dose[i],cex=0.7)
      }
    }
    if (ks0>0 & ks1>0 & ks2==0){
      for(i in 1:ks1){
        text(-16.5, -0.3+i+1+sliderDose, pos=4, analysis_rob_13$dose[i],cex=0.7)
      }
      for(i in (ks1+1):(ks0+ks1)){
        text(-16.5, -0.3+i+2+sliderDose, pos=4, analysis_rob_13$dose[i],cex=0.7)
      }
    }
    if (ks0>0 & ks1==0 & ks2>0){
      for(i in 1:ks2){
    ...
    if (ks0>0 & ks1>0 & ks2>0){
      ...
      for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
        text(-16.5, -0.3+i+4+sliderDose, pos=4, analysis_rob_13$dose[i],cex=0.7)
      }
    }
  }
  
  
  count=sum(str_count(analysis_rob_13$dose, "\\*"),na.rm = TRUE)
  ...
  if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
    for(i in 1:result_13$k){
      ...
      text(-28, 0.5+i, pos=4, analysis_rob_13$severity[i])
      }
    }
  }
  if (ks0>0 & ks1>0 & ks2==0){
    for(i in 1:ks1){
      if(hide_severity=="FALSE"){
      text(-28, 0.5+i+1, pos=4, analysis_rob_13$severity[i])
      }
    }
    for(i in (ks1+1):(ks0+ks1)){
      text(-28, 0.5+i+2, pos=4, analysis_rob_13$severity[i])
    }
  }
  if (ks0>0 & ks1==0 & ks2>0){
    for(i in 1:ks2){
  ...
  if (ks0>0 & ks1>0 & ks2>0){
    ...
    for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
      if(hide_severity=="FALSE"){
      text(-28, 0.5+i+4, pos=4, analysis_rob_13$severity[i])
      }
    }
  }
  ...
  if(result_13$k>1){
    ...
    if(length(unique(analysis_data_13$subgroup))>1){
      ...
      text(-28, 0.2, pos=4, cex=0.9, bquote(paste("Heterogeneity: Q = ",
                                                  .(formatC(result_13$QE, digits=2, format="f")),
                                                  ", p = ", .(formatC(result_13$QEp, digits=2, format="f")), "; ", I^2, " = ",
                                                  .(formatC(rresult_13$I2, digits=1, format="f")), "%; ",tau^2," = ",
                                                  .(formatC(result_13$tau2, digits=2, format="f")), "; ", "Test for subgroup differences: Q = ",
  ...
  else if (nrow(analysis_data_13)==0){

...
forest_Time_to_neg_conv=function(data,dat,comp.num,sliderRob=0,sliderDose=0,high_RoB=FALSE,keep_only="All population",model="Random-effects",
  ...
  data=data[,c("Trial_ID","ID_stats","name_study","First_author","Year","Study_design","Funding","Conflict_of_interest_category",
               ...
               "Time_to_NEG_conv_HR","Time_to_NEG_conv_HR_LowerCI","Time_to_NEG_conv_HR_UpperCI",
               "ROB_1_randomization","ROB2_Time_neg_conv","ROB_2_Deviations_from_intervention","ROB_3_Time_to_NEG_conv",
               "ROB_4__Viral_Neg_Conv_events","ROB_5_Time_to_NEG_conv","ROB_6_Time_to_NEG_conv","Type_publication"
  ...
  data$ROB2_Time_neg_conv=ifelse(is.na(data$ROB2_Time_neg_conv),data$ROB_2_Deviations_from_intervention,data$ROB2_Time_neg_conv)
...
sev=sev1
  ...
  analysis_data_13=cbind.data.frame(Trial_ID=new_data$Trial_ID,First_author=new_data$First_author,
                         ...
                         D2=as.character(new_data$ROB2_Time_neg_conv),
                         D3=as.character(new_data$ROB_3_Time_to_NEG_conv),
                         D4=as.character(new_data$ROB_4__Viral_Neg_Conv_events),
                         D5=as.character(new_data$ROB_5_Time_to_NEG_conv),
  ...
  interval=paste("[",format(round(analysis_data_13$LCI,digits = 2),nsmall = 2),", ",format(round(analysis_data_13$UCI,digits = 2),nsmall = 2),"]",sep="")
  analysis_data_13$ress=paste(format(round(analysis_data_13$HR,digits = 2),nsmall = 2),interval,sep=" ")
  analysis_data_13$ress=ifelse(analysis_data_13$ress=="NA [NA, NA]","",analysis_data_13$ress)
  ...
  if(keep_only=="Mild population"){
    data_sev<- analysis_data_13 %>%
      filter(analysis_data_13$sev1==1)
    analysis_data_13=data_sev
  }
  
  if(keep_only=="Mixed population"){
    
    data_sev<-analysis_data_13 %>%
      filter(analysis_data_13$sev1 %in% c(2:11))
  ...
  if(keep_only=="Critical population"){
    ...
    data_sev<-analysis_data_13 %>%
      filter(analysis_data_13$sev1==12)
    
    analysis_data_13=data_sev
  }
  
  exclude_preprint<- analysis_data_13 %>%
    filter(analysis_data_13$Publication=="published paper")
  
  
  
  if(Publication==TRUE){
    analysis_data_13<-exclude_preprint}
  ...
  analysis_data_13=as.data.frame(analysis_data_13)
  if(nrow(analysis_data_13)>0){
  
  ################################################################
  ################################################################
    exclude_rob<- analysis_data_13 %>%
      filter(analysis_data_13$Overall=="High")
    
    exclude_rob1<- analysis_data_13 %>%
      filter(analysis_data_13$Overall %in% c("High","Some concerns") )
    
    w=setdiff(analysis_data_13$First_author,exclude_rob$First_author)
    
    w1=setdiff(analysis_data_13$First_author,exclude_rob1$First_author)
    
    if(high_RoB=="High RoB"){
      analysis_data_13<-subset(analysis_data_13,analysis_data_13$First_author %in% w)}
    
    
    if(high_RoB=="High RoB/Some concerns"){
      analysis_data_13<-subset(analysis_data_13,analysis_data_13$First_author %in% w1)}
  ...
  analysis_data_13=analysis_data_13[order(analysis_data_13$sev,decreasing=TRUE),]
  
  ilab=cbind(F_U_days=as.character(analysis_data_13$F_U_days),Intervention1=as.character(analysis_data_13$Intervention1),
             Intervention2=as.character(analysis_data_13$Intervention2),
             counts1=analysis_data_13$N1,counts2=analysis_data_13$N2,analysis_data_13$ress)
  ilab=as.data.frame(ilab)
  
  ########################################################################
  analysis_data_13_0<-subset(analysis_data_13,analysis_data_13$comp==comp.num & analysis_data_13$subgroup==0)
  analysis_data_13_1<-subset(analysis_data_13,analysis_data_13$comp==comp.num & analysis_data_13$subgroup==1)
  analysis_data_13_2<-subset(analysis_data_13,analysis_data_13$comp==comp.num & analysis_data_13$subgroup==2)
  ##########################################################################
  
  
  result_13=rma(measure="GEN",yi=analysis_data_13$logHR,sei=analysis_data_13$se,
                slab=paste(analysis_data_13$First_author,analysis_data_13$Year,sep=", "),
                subset=(analysis_data_13$comp==comp.num),method = het_estimate)
  
  
  
  
  result_131=metagen(TE=analysis_data_13$logHR,seTE=analysis_data_13$se,byvar = analysis_data_13$subgroup,method.tau = het_estimate)
  
  if(model=="Fixed-effects"){
    
    result_13=update(result_13,method = "FE")
  }
  
  pred_13<- predict(result_13, transf=exp, digits=2)
  pred_13 <- formatC(c(pred_13$pred, pred_13$ci.lb, pred_13$ci.ub), format="f", digits=2)
  
  
  ks0=0
  ks1=0
  ks2=0
  if (result_13$k>1 & nrow(analysis_data_13_0)>0){
    result_13.0=rma(measure="GEN",yi=analysis_data_13_0$logHR,sei=analysis_data_13_0$se,
                    slab=paste(analysis_data_13_0$First_author,analysis_data_13_0$Year,sep=", "),method = het_estimate)
    
    if(model=="Fixed-effects"){
      result_13.0=update(result_13.0,method = "FE")
      
    }
    
    ks0=result_13.0$k
    
    pred_13.0<- predict(result_13.0, transf=exp, digits=2)
    pred_13.0 <- formatC(c(pred_13.0$pred, pred_13.0$ci.lb, pred_13.0$ci.ub), format="f", digits=2)
    
  }
  if (result_13$k>1 & nrow(analysis_data_13_1)>0){
    result_13.1=rma(measure="GEN",yi=analysis_data_13_1$logHR,sei=analysis_data_13_1$se,
                    slab=paste(analysis_data_13_1$First_author,analysis_data_13_1$Year,sep=", "),method = het_estimate)
    
    if(model=="Fixed-effects"){
      result_13.1=update(result_13.1,method = "FE")
      
    }
    
    ks1=result_13.1$k
    
    pred_13.1<- predict(result_13.1, transf=exp, digits=2)
    pred_13.1<- formatC(c(pred_13.1$pred, pred_13.1$ci.lb, pred_13.1$ci.ub), format="f", digits=2)
    
  }
  if (result_13$k>1 & nrow(analysis_data_13_2)>0 ){
    result_13.2=rma(measure="GEN",yi=analysis_data_13_2$logHR,sei=analysis_data_13_2$se,
                    slab=paste(analysis_data_13_2$First_author,analysis_data_13_2$Year,sep=", "),method = het_estimate)
    
    if(model=="Fixed-effects"){
      
      result_13.2=update(result_13.2,method = "FE")
    }
    
    
    ks2=result_13.2$k
    
    pred_13.2<- predict(result_13.2, transf=exp, digits=2)
    pred_13.2<- formatC(c(pred_13.2$pred, pred_13.2$ci.lb, pred_13.2$ci.ub), format="f", digits=2)
  ...
  result_13.3=result_13
  
  data_rob=analysis_data_13 %>%
  ...
  analysis_data_13$var_hr=analysis_data_13$se*analysis_data_13$se
  analysis_data_13$weights=1/(analysis_data_13$var_hr+result_13$tau2)
  
  analysis_data_13$relative_weights=(analysis_data_13$weights/sum(analysis_data_13$weights,na.rm = T))*100
  
  analysis_data_13$relative_weights=format(round(analysis_data_13$relative_weights,digits = 2),nsmall=2)
  analysis_data_13$relative_weights=trimws(analysis_data_13$relative_weights)
  
  analysis_data_13$relative_weights=ifelse(analysis_data_13$relative_weights=="NA","",analysis_data_13$relative_weights)
  
  analysis_data_13$relative_weights=ifelse(analysis_data_13$relative_weights!="",
                                           paste(analysis_data_13$relative_weights,"%",sep = ""),analysis_data_13$relative_weights)
  
  ilab$weights=analysis_data_13$relative_weights
  ilab$weights=ifelse(ilab$weights!=" 0.00%",ilab$weights," ")
  
  
  forest=rob_append_to_forest(result_13,atransf=exp,showweights=F, xlab="Hazard Ratio",cex=0.8,mlab="",
  ...
  addpoly.rma(result_13.3,row=-1,mlab="",col="darkblue",border="darkblue",atransf=exp,cex=0.9,annotate = F)
  
  par(font=2)
  text(-5.1,-1,paste(pred_13[1], " [", pred_13[2], ",  ", pred_13[3], "]",sep = ""),cex = cex1)
  par(font=1)
...
if(sub!="No subgroup analysis"){
  if (result_13$k>1){
    if (ks0>0 & ks1>0 & ks2==0){
      if(sub=="Severity"){
        addpoly.rma(result_13.1,row=1,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.0,row=ks1+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Conflicts of interest"){
        addpoly.rma(result_13.1,row=1,mlab="                    Studies with conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.0,row=ks1+2,mlab="                    Studies without conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_13.1,row=1,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.0,row=ks1+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Location"){
        addpoly.rma(result_13.1,row=1,mlab="                    National studies",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.0,row=ks1+2,mlab="                    Multinational studies",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Separate Standard care/Placebo"){
        addpoly.rma(result_13.1,row=1,mlab="                    Placebo summary result",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.0,row=ks1+1.8,mlab="                    Standard care summary result",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        
      }
      
      
      par(font=2)
      text(-5.1,1,paste(pred_13.1[1], " [", pred_13.1[2], ",  ",pred_13.1[3], "]",sep = ""),cex = cex1)
      text(-5.1,ks1+1.8,paste(pred_13.0[1], " [", pred_13.0[2], ",  ", pred_13.0[3], "]",sep = ""),cex = cex1)
      par(font=1)
    ...
    if (ks0>0 & ks1==0 & ks2>0){
      if(sub=="Severity"){
        addpoly.rma(result_13.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.0,row=ks2+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_13.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.0,row=ks2+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
    ...
    if (ks0==0 & ks1>0 & ks2>0){
      if(sub=="Severity"){
        addpoly.rma(result_13.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      if(sub=="Funding"){
        addpoly.rma(result_13.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      text(-5.1,1,paste(pred_13.2[1], " [", pred_13.2[2], ",  ", pred_13.2[3], "]",sep = ""),cex = cex1)
      text(-5.1,ks2+2,paste(pred_13.1[1], " [", pred_13.1[2], ",  ", pred_13.1[3], "]",sep = ""),cex = cex1)
    ...
    if (ks0>0 & ks1>0 & ks2>0){
      if(sub=="Severity"){
        addpoly.rma(result_13.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.0,row=ks2+4+ks1,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      if(sub=="Funding"){
        addpoly.rma(result_13.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_13.0,row=ks2+4+ks1,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      ...
      text(-5.1,1,paste(pred_13.2[1], " [", pred_13.2[2], ",  ", pred_13.2[3], "]",sep = ""),cex = cex1)
      text(-5.1,ks2+2,paste(pred_13.1[1], " [", pred_13.1[2], ",  ", pred_13.1[3], "]",sep = ""),cex = cex1)
      text(-5.1,ks2+4,paste(pred_13.0[1], " [", pred_13.0[2], ",  ", pred_13.0[3], "]",sep = ""),cex = cex1)
  ...
  text(-4.5,3.3+max(allrows),"Time to negative conversion",
       pos=4,
       cex=1)
  ...
  if(hide_dose=="FALSE"){
    if(comp.num==comp.num){
      if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
        for(i in 1:result_13$k){
          text(-11.5, -0.3+i+sliderDose, pos=4, analysis_data_13$dose[i],cex=cex1-0.3)
        }
      }
      if (ks0>0 & ks1>0 & ks2==0){
        for(i in 1:ks1){
          text(-11.5, -0.3+i+1+sliderDose, pos=4, analysis_data_13$dose[i],cex=cex1-0.3)
        }
        for(i in (ks1+1):(ks0+ks1)){
          text(-11.5, -0.3+i+2+sliderDose, pos=4, analysis_data_13$dose[i],cex=cex1-0.3)
        }
      }
      if (ks0>0 & ks1==0 & ks2>0){
        for(i in 1:ks2){
          text(-11.5, -0.3+i+1+sliderDose, pos=4, analysis_data_13$dose[i],cex=cex1-0.2)
        }
        for(i in (ks2+1):(ks0+ks2)){
          text(-11.5, -0.3+i+2+sliderDose, pos=4, analysis_data_13$dose[i],cex=cex1-0.2)
        }  
      }
      if (ks0==0 & ks1>0 & ks2>0){
        for(i in 1:ks2){
      ...
      if (ks0>0 & ks1>0 & ks2>0){
        ...
        for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
          text(-11.5, -0.3+i+4+sliderDose, pos=4, analysis_data_13$dose[i],cex=cex1-0.2)
        }
      }
    }
    
    
    count=sum(str_count(analysis_data_13$dose, "\\*"),na.rm = TRUE)
  ...
  if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
    for(i in 1:result_13$k){
      if(hide_severity=="FALSE"){
        text(-15, 0.5+i, pos=4, analysis_data_13$severity[i],cex=cex1-0.1)
      }
    }
  }
  if (ks0>0 & ks1>0 & ks2==0){
    for(i in 1:ks1){
      if(hide_severity=="FALSE"){
        text(-15, 0.5+i+1, pos=4, analysis_data_13$severity[i],cex=cex1-0.1)
      }
    }
    for(i in (ks1+1):(ks0+ks1)){
      if(hide_severity=="FALSE"){
        text(-15, 0.5+i+2, pos=4, analysis_data_13$severity[i],cex=cex1-0.1)
      }
    }
  }
  if (ks0>0 & ks1==0 & ks2>0){
  ...
  if (ks0>0 & ks1>0 & ks2>0){
    ...
    for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
      if(hide_severity=="FALSE"){
        text(-15, 0.5+i+4, pos=4, analysis_data_13$severity[i],cex=cex1-0.1)
      }
    }
  }
  ...
  if(nrow(analysis_data_13)>1){
    
    
    N1_total=sum(analysis_data_13$N1)
    N2_total=sum(analysis_data_13$N2)
    # 
  ...
  af=paste(a1,a2,sep ="\n")
  if(nrow(analysis_data_13)>7.5){
    legend(0.5,-4,legend=af,
           nc.5ol=1,cex=0.75,box.lty = 0,text.col = "blue")
    
    
  }else{
    legend(0,-3,legend=af,
           ncol=1,cex=0.75,box.lty = 0,text.col = "blue")}
  
  
  if(result_13$k>8.5){
    if(length(unique(analysis_data_13$subgroup))==1){
      par(font=2)
      text(-15,-1.5 , pos=4, cex=cex1, bquote(paste("Heterogeneity results: Q = ",
                                                    .(formatC(result_13$QE, digits=2, format="f")),
                                                    ", p = ", .(formatC(result_13$QEp, digits=2, format="f")), "; ", I^2, " = ",
                                                    .(formatC(result_13$I2, digits=1, format="f")), "%; ",tau^2," = ",
                                                    .(formatC(result_13$tau2, digits=2, format="f")))))  
    }
    if(length(unique(analysis_data_13$subgroup))>1){
      par(font=2)
      ...
      text(-15, -2.5, pos=4, cex=cex1, bquote(paste("Test for subgroup differences: Q = ",
                                                    .(formatC(result_131$Q.b.random, digits=2, format="f")),"; ", "p = ",
                                                    .(formatC(result_131$pval.Q.b.random, digits=2, format="f"))
  ...
  if((result_13$k>1) & (result_13$k<9)){
    if(length(unique(analysis_data_13$subgroup))==1){
      par(font=2)
      text(-15,-1.5 , pos=4, cex=cex1, bquote(paste("Heterogeneity results: Q = ",
                                                    .(formatC(result_13$QE, digits=2, format="f")),

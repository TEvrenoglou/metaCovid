...
forest_mortality=function(data,
                          type,
                          slider=0,
                        subgroup=T,
                        overall=F,
  ...
  data=subset(data, 
              Trial_ID!=66 & Trial_ID!=5015 & Trial_ID!= 47 & Trial_ID!= 67 & Trial_ID!= 1012 & Trial_ID!= 78 & Trial_ID!= 70
              & Trial_ID!= 104 
              & Trial_ID!= 5010 & Trial_ID!= 75 & Trial_ID!= 105)
...
data$n_Randomized=data$mortality_denominator
...
data$F_U_days=ifelse(is.na(data$All_cause_mort_events_TP),data$F_U_months,as.numeric(data$All_cause_mort_events_TP))
...
data$n_Randomized=as.numeric(data$mortality_denominator)
...
data$All_cause_mortality_events_D1428=data$All_cause_mortality_events
...
analysis_data_05_14=cbind.data.frame(Trial_ID=new_data_05_14$Trial_ID,First_author=new_data_05_14$First_author,
                                     ...
                                     B=as.character(new_data_05_14$ROB2_mortality),
                                     C=as.character(new_data_05_14$ROB_3_mortality_),
                                     D=as.character(new_data_05_14$ROB4_mortality_),
                                     E=as.character(new_data_05_14$ROB_5_mortality_),
                                     Overall=as.character(new_data_05_14$ROB6_mortality),
...
slab=ifelse(is.na(analysis_data_05_14$complement),slab,paste(slab,analysis_data_05_14$complement,sep = ""))
...
cex1=0.9
  ...
  forest=rob_append_to_forest(res,atransf=exp,
                       ...
                       #alim=c(floor(result_05_14$ci.lb)+expand[1],ceiling(result_05_14$ci.ub))+expand[2],
                       at=log(c(0.1,1,5)),
                       steps=4,
...
text(-2.4,3.1+max(rows),"All-cause mortality",
     pos=4,
     cex=1)

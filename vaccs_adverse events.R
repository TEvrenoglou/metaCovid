forest_AE_vaccs=function(data,
  ...
  )
  {
    ...
    data=subset(data,
                Trial_ID!=66 & Trial_ID!=5015 & Trial_ID!= 47 & Trial_ID!= 67 & Trial_ID!= 73 & Trial_ID!= 5025
                & Trial_ID!= 78 & Trial_ID!= 1012 & Trial_ID!= 70
                & Trial_ID!= 104 & Trial_ID!= 5010 & Trial_ID!= 105
    )
    ...
    data$n_Randomized=data$AE_denominator
    ...
    data$F_U_days=ifelse(is.na(data$AE_TP),data$F_U_months,as.numeric(data$AE_TP))
    ...
    data$n_Randomized=as.numeric(data$AE_denominator)
    ...
    data$Total_AE=ifelse(is.na(data$Total_AE),as.numeric(data$Solicited_AE),as.numeric(data$Total_AE))
    data$Total_AE=ifelse(is.na(data$Total_AE),as.numeric(data$unsolicited_AE),as.numeric(data$Total_AE))
    ...
    pair_data_05_14=pairwise_old(studlab=Trial_ID,treat=Treat_Name,event=Total_AE_D1428,n=n_Randomized,
    ...
    analysis_data_05_14=cbind.data.frame(Trial_ID=new_data_05_14$Trial_ID,First_author=new_data_05_14$First_author,
                                         ...
                                         B=as.character(new_data_05_14$ROB2_Adverse_events),
                                         C=as.character(new_data_05_14$ROB_3_Adverse_events),
                                         D=as.character(new_data_05_14$ROB_4_Adverse_events),
                                         E=as.character(new_data_05_14$ROB_5_Adverse_events),
                                         Overall=as.character(new_data_05_14$ROB_6_Adverse_events),
    ...
    analysis_data_05_14$wi=1/(analysis_data_05_14$seTE*analysis_data_05_14$seTE)
    
    analysis_data_05_14$wi=ifelse(analysis_data_05_14$oo=="0-0",NA,analysis_data_05_14$wi)
  ...
  if(nrow(analysis_data_05_14)>1){
    res <- rma.mv(yi, vi, random = ~ 1 | as.factor(sub), data =analysis_data_05_14,
  ...
  if(nrow(analysis_data_05_14)>1){
    res <- rma(yi, vi, data =analysis_data_05_14,
               slab=analysis_data_05_14$slab,drop00 = F)
  ...
  for(i in 1:nrow(analysis_data_05_14)){
    if(analysis_data_05_14$oo[i]=="0-0"){
      legend(-2.5+box, rows[i]+0.3, "                                                                                                                                                                             ", 
             box.col = "white", bg = "white", adj = 0.5,cex=0.6)
    }
  }
...
text(-2.5,3.5+max(rows),"Any adverse event (AE)",
     pos=4,
     cex=1)
  # 
  ...
  if(type=="Non replicating viral vector"){
    ...
    text(-15, -1.5, "MenACWY: Quadrivalent meningococcal conjugate vaccine", pos=4, cex=0.7)

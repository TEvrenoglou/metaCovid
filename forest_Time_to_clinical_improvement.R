...
forest_Time_to_clinical_improvement=function(data,comp.num,sliderRob=0,sliderDose=0,high_RoB=FALSE,keep_only="All population",model="Random-effects",
  ...
  data=data[,c("Trial_ID","ID_stats","name_study","First_author","Year","Study_design","Funding","Conflict_of_interest_category",
               ...
               "ROB_4_Clinical_improvement","ROB_5_Time_to_clinical_improvement","ROB_6__Time_to_clinical_improvement"
 ...
 # data$NEG_conv_denominator=ifelse(is.na(data$NEG_conv_denominator),data$n_Randomized,data$NEG_conv_denominator)
  ...
  #analysis_data_09$N1=as.numeric(levels(analysis_data_09$N1))[analysis_data_09$N1]
  #analysis_data_09$N2=as.numeric(levels(analysis_data_09$N2))[analysis_data_09$N2]
  #analysis_data_09$comp=as.numeric(levels(analysis_data_09$comp))[analysis_data_09$comp]
  
  #analysis_data_09$LCI=as.numeric(levels(analysis_data_09$LCI))[analysis_data_09$LCI]
  #analysis_data_09$UCI=as.numeric(levels(analysis_data_09$UCI))[analysis_data_09$UCI]
  
  #analysis_data_09$HR=as.numeric(levels(analysis_data_09$HR))[analysis_data_09$HR]
  #analysis_data_09$N1=as.numeric(analysis_data_09$N1)
  #analysis_data_09$N2=as.numeric(analysis_data_09$N2)
  #analysis_data_09$comp=as.numeric(analysis_data_09$comp)
  #analysis_data_09$LCI=as.numeric(analysis_data_09$LCI)
  #analysis_data_09$UCI=as.numeric(analysis_data_09$UCI)
  #analysis_data_09$HR=as.numeric(analysis_data_09$HR)
  
  analysis_data_09$logHR=log(analysis_data_09$HR)
  analysis_data_09$se=(log(analysis_data_09$UCI)-log(analysis_data_09$LCI))/3.92
  
  #analysis_data_09$sev=as.numeric(levels(analysis_data_09$sev))[analysis_data_09$sev]
  #analysis_data_09$subgroup=as.numeric(levels(analysis_data_09$subgroup))[analysis_data_09$subgroup]
  ...
  comp.num=comp.num
  #file_name="comp.num_09_pharma_rct.jpeg"
  #file_name=paste(comp.num,"_09_pharma_rct.jpeg",sep="")
  #jpeg(file=file_name,width=2431,height=1346,res=200)
  #meta-analysis#
  #rm(result_09)
  #rm(result_09.3)
  #analysis_data_09=subset(analysis_data_09_all,comp==comp.num & !is.na(logHR) & !is.na(se))
  ...
  ilab=cbind(F_U_days=as.character(analysis_data_09$F_U_days),Intervention1=as.character(analysis_data_09$Intervention1),
             ...
             counts1=analysis_data_09$N1,counts2=analysis_data_09$N2)
  ...
  result_09=rma(measure="GEN",yi=analysis_data_09$logHR,sei=analysis_data_09$se,
                ...
                subset=(analysis_data_09$comp==comp.num))
  
  result_091=metagen(TE=analysis_data_09$logHR,seTE=analysis_data_09$se,byvar = analysis_data_09$subgroup)
  
  
  if(model=="Fixed-effects"){
    result_09=update(result_09,tau2=0)
  ...
  if (result_09$k>1  & nrow(analysis_data_09_0)>0){
    result_09.0=rma(measure="GEN",yi=analysis_data_09_0$logHR,sei=analysis_data_09_0$se,
                    slab=paste(analysis_data_09_0$First_author,analysis_data_09_0$Year,sep=", "))
    if(model=="Fixed-effects"){
      result_09.0=update(result_09.0,tau2=0)
  ...
  if (result_09$k>1 & nrow(analysis_data_09_1)>0){
    result_09.1=rma(measure="GEN",yi=analysis_data_09_1$logHR,sei=analysis_data_09_1$se,
                    slab=paste(analysis_data_09_1$First_author,analysis_data_09_1$Year,sep=", "))
    if(model=="Fixed-effects"){
      result_09.1=update(result_09.1,tau2=0)
  ...
  if (result_09$k>1 & nrow(analysis_data_09_2)>0){
    result_09.2=rma(measure="GEN",yi=analysis_data_09_2$logHR,sei=analysis_data_09_2$se,
                    slab=paste(analysis_data_09_2$First_author,analysis_data_09_2$Year,sep=", "))
    
    if(model=="Fixed-effects"){
      result_09.2=update( result_09.2,tau2=0)
    }
    ks2=result_09.2$k
  }
  ...
  metafor::forest(result_09,atransf=exp,showweights=TRUE, xlab="Hazard Ratio",header=c("Study                                   Study Duration         Intervention 1               Intervention 2          N1       N2                                                                 Risk of Bias
                                                    days                                                                                                                                                                A       B       C       D       E       Overall"),cex=0.73,mlab="",
                  col="white",border="white",xlim=c(-28,20),alim=c(floor(result_09$ci.lb)-2,ceiling(result_09$ci.ub))+1,steps=4,
  ...
  addpoly.rma(result_09.3,row=-1,mlab="",col="darkblue",border="darkblue",atransf=exp,cex=0.9)
  if (result_09$k>1){
    if (ks0>0 & ks1>0 & ks2==0){
      if(sub=="Severity"){
      addpoly.rma(result_09.1,row=1,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_09.0,row=ks1+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9)}
      
      if(sub=="Conflicts of interest"){
        addpoly.rma(result_09.1,row=1,mlab="                    Studies with conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_09.0,row=ks1+2,mlab="                    Studies without conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9)}
      
      if(sub=="Funding"){
        addpoly.rma(result_09.1,row=1,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_09.0,row=ks1+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      
      if(sub=="Location"){
        addpoly.rma(result_09.1,row=1,mlab="                    National studies",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_09.0,row=ks1+2,mlab="                    Multinational studies",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      
    }
    if (ks0>0 & ks1==0 & ks2>0){
      if(sub=="Severity"){
      addpoly.rma(result_09.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_09.0,row=ks2+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      if(sub=="Funding"){
        addpoly.rma(result_09.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_09.0,row=ks2+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
    ...
    if (ks0==0 & ks1>0 & ks2>0){
      if(sub=="Severity"){
      addpoly.rma(result_09.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_09.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_09.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_09.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
    ...
    if (ks0>0 & ks1>0 & ks2>0){
      ...
      addpoly.rma(result_09.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_09.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9)
      addpoly.rma(result_09.0,row=ks2+4+ks1,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9)
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_09.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_09.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9)
        addpoly.rma(result_09.0,row=ks2+4+ks1,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9)
      }
  ...
  text(-6,3.5+max(allrows),"Pharmacological treatments",
       ...
       cex=1,
       col = "darkblue")
  text(-6,3+max(allrows),"Time to clinical improvement",
       pos=4,
       cex=1)
  
  #get rob data#
  rob_data_09=cbind(Trial_ID=data$Trial_ID,A=as.character(data$ROB_1_randomization),
                    B=ifelse(is.na(data$ROB2_Time_clinic_imp),as.character(data$ROB_2_Deviations_from_intervention),as.character(data$ROB2_Time_clinic_imp)),
                    C=as.character(data$ROB_3__Time_to_clinical_improvement),D=as.character(data$ROB_4_Clinical_improvement),E=as.character(data$ROB_5_Time_to_clinical_improvement),
                    Overall=as.character(data$ROB_6__Time_to_clinical_improvement))
  rob_data_09=as.data.frame(rob_data_09)
  ncomp=length(analysis_data_09$Trial_ID)
  A=rep("NA",ncomp)
  B=rep("NA",ncomp)
  C=rep("NA",ncomp)
  D=rep("NA",ncomp)
  ...
  nst=length(rob_data_09$Trial_ID)
  for(i in 1:ncomp){
    for(j in 1:nst){
      if(rob_data_09$Trial_ID[j]==analysis_data_09$Trial_ID[i]){
        A[i]=as.character(rob_data_09$A[j])
        B[i]=as.character(rob_data_09$B[j])
        C[i]=as.character(rob_data_09$C[j])
        D[i]=as.character(rob_data_09$D[j])
        E[i]=as.character(rob_data_09$E[j])
        Overall[i]=as.character(rob_data_09$Overall[j])
      }   
    }
  }
  A=trimws(A)
  ...
  analysis_data_09$A=A
  analysis_data_09$B=B
  analysis_data_09$C=C
  analysis_data_09$D=D
  analysis_data_09$E=E
  analysis_data_09$Overall=Overall
  analysis_data_09$A[analysis_data_09$A=="High"]="red"
  analysis_data_09$A[analysis_data_09$A=="Low"]="chartreuse3"
  analysis_data_09$A[analysis_data_09$A=="Some concerns"]="gold"
  analysis_data_09$B[analysis_data_09$B=="High"]="red"
  analysis_data_09$B[analysis_data_09$B=="Low"]="chartreuse3"
  analysis_data_09$B[analysis_data_09$B=="Some concerns"]="gold"
  analysis_data_09$C[analysis_data_09$C=="High"]="red"
  analysis_data_09$C[analysis_data_09$C=="Low"]="chartreuse3"
  analysis_data_09$C[analysis_data_09$C=="Some concerns"]="gold"
  analysis_data_09$D[analysis_data_09$D=="High"]="red"
  analysis_data_09$D[analysis_data_09$D=="Low"]="chartreuse3"
  analysis_data_09$D[analysis_data_09$D=="Some concerns"]="gold"
  analysis_data_09$E[analysis_data_09$E=="High"]="red"
  analysis_data_09$E[analysis_data_09$E=="Low"]="chartreuse3"
  analysis_data_09$E[analysis_data_09$E=="Some concerns"]="gold"
  analysis_data_09$Overall[analysis_data_09$Overall=="High"]="red"
  analysis_data_09$Overall[analysis_data_09$Overall=="Low"]="chartreuse3"
  analysis_data_09$Overall[analysis_data_09$Overall=="Some concerns"]="gold"
  #analysis_rob_09=subset(analysis_data_09,comp==comp.num & !is.na(logHR) & !is.na(se))
  
  analysis_rob_09= analysis_data_09 %>%
    filter(analysis_data_09$comp==comp.num)
  
  analysis_rob_09=analysis_rob_09[complete.cases(analysis_rob_09$UCI), ]
  ...
  if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
    for(i in 1:result_09$k){
      legend(x=3.2,y=0.41+i+sliderRob,legend=c("", "","","","",""),
             col=c(analysis_rob_09$A[i],analysis_rob_09$B[i],analysis_rob_09$C[i],analysis_rob_09$D[i],analysis_rob_09$E[i],analysis_rob_09$Overall[i]), 
             cex=0.75,inset=0.1,bg="white",
             box.lty=0,title = "",fill = c(analysis_rob_09$A[i],analysis_rob_09$B[i],analysis_rob_09$C[i],analysis_rob_09$D[i],analysis_rob_09$E[i],analysis_rob_09$Overall[i]),pt.cex = 5
             ,x.intersp=0.5,
             horiz = TRUE,
             border = c(analysis_rob_09$A[i],analysis_rob_09$B[i],analysis_rob_09$C[i],analysis_rob_09$D[i],analysis_rob_09$E[i],analysis_rob_09$Overall[i]),
  ...
  if(comp.num==comp.num){
    if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
      for(i in 1:result_09$k){
        text(-16.5, -0.3+i+sliderDose, pos=4, analysis_rob_09$dose[i],cex=0.7)
      }
    }
    if (ks0>0 & ks1>0 & ks2==0){
      for(i in 1:ks1){
        text(-16.5, -0.3+i+1+sliderDose, pos=4, analysis_rob_09$dose[i],cex=0.7)
      }
      for(i in (ks1+1):(ks0+ks1)){
        text(-16.5, -0.3+i+2+sliderDose, pos=4, analysis_rob_09$dose[i],cex=0.7)
      }
    }
    if (ks0>0 & ks1==0 & ks2>0){
      for(i in 1:ks2){
    ...
    if (ks0>0 & ks1>0 & ks2>0){
      ...
      for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
        text(-16.5, -0.3+i+4+sliderDose, pos=4, analysis_rob_09$dose[i],cex=0.7)
      }
    }
  }
  
  count=sum(str_count(analysis_rob_09$dose, "\\*"),na.rm = TRUE)
  ...
  if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
    for(i in 1:result_09$k){
      text(-28, 0.5+i, pos=4, analysis_rob_09$severity[i])
    }
  }
  if (ks0>0 & ks1>0 & ks2==0){
    for(i in 1:ks1){
      if(hide_severity=="FALSE"){
      text(-28, 0.5+i+1, pos=4, analysis_rob_09$severity[i])
      }
    }
    for(i in (ks1+1):(ks0+ks1)){
      text(-28, 0.5+i+2, pos=4, analysis_rob_09$severity[i])
    }
  }
  if (ks0>0 & ks1==0 & ks2>0){
    for(i in 1:ks2){
  ...
  if (ks0>0 & ks1>0 & ks2>0){
    ...
    for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
      if(hide_severity=="FALSE"){
      text(-28, 0.5+i+4, pos=4, analysis_rob_09$severity[i])
      }
    }
  }
  ...
  if(result_09$k>1){
    ...
    if(length(unique(analysis_data_09$subgroup))>1){
      ...
      text(-28, 0.2, pos=4, cex=0.9, bquote(paste("Heterogeneity: Q = ",
                                                  ...
                                                  ", p = ", .(formatC(result_09$QEp, digits=2, format="f")), "; ", I^2, " = ",
                                                  .(formatC(result_09$I2, digits=1, format="f")), "%; ",tau^2," = ",
                                                  .(formatC(result_09$tau2, digits=2, format="f")), "; ", "Test for subgroup differences: Q = ",
  ...
  else if (nrow(analysis_data_09)==0){

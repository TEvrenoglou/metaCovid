forest_Time_to_clinical_improvement=function(data,dat,comp.num,sliderRob=0,sliderDose=0,high_RoB=FALSE,keep_only="All population",model="Random-effects",
  ...
  data=data[,c("Trial_ID","ID_stats","name_study","First_author","Year","Study_design","Funding","Conflict_of_interest_category",
               ...
               "Time_to_clinical_improvement_HR","Time_to_clinical_improvement_HR_CI_Lower","Time_to_clinical_improvement_HR_CI_Upper",
               "ROB_1_randomization","ROB2_Time_clinic_imp","ROB_2_Deviations_from_intervention","ROB_3__Time_to_clinical_improvement",
               "ROB_4_Clinical_improvement","ROB_5_Time_to_clinical_improvement","ROB_6__Time_to_clinical_improvement","Type_publication"
  ...
  data$ROB2_Time_clinic_imp=ifelse(is.na(data$ROB2_Time_clinic_imp),data$ROB_2_Deviations_from_intervention,data$ROB2_Time_clinic_imp)
  ...
  if(
    (is.null(pair_data$Time_to_clinical_improvement_HR)) & (is.null(pair_data$Time_to_clinical_improvement_HR_CI_Lower)) &
     (is.null(pair_data$Time_to_clinical_improvement_HR_CI_Upper))
     ){
    
    pair_data$Time_to_clinical_improvement_HR=pair_data$Time_to_clinical_improvement_HR1
    pair_data$Time_to_clinical_improvement_HR_CI_Lower=pair_data$Time_to_clinical_improvement_HR_CI_Lower1
    pair_data$Time_to_clinical_improvement_HR_CI_Upper=pair_data$Time_to_clinical_improvement_HR_CI_Upper1
  }
     
  
  pair_data$Time_to_clinical_improvement_HR=as.numeric(pair_data$Time_to_clinical_improvement_HR)
  pair_data$Time_to_clinical_improvement_HR_CI_Lower=as.numeric(pair_data$Time_to_clinical_improvement_HR_CI_Lower)
  pair_data$Time_to_clinical_improvement_HR_CI_Upper=as.numeric(pair_data$Time_to_clinical_improvement_HR_CI_Upper)
 ...
 # new_data$Treat_Dose_desc=ifelse(is.null(new_data$Treat_Dose_desc),new_data$Treat_Dose_desc1,new_data$Treat_Dose_desc)
  ...
  analysis_data_09=cbind.data.frame(Trial_ID=new_data$Trial_ID,First_author=new_data$First_author,
                         ...
                         D2=as.character(new_data$ROB2_Time_clinic_imp),
                         D3=as.character(new_data$ROB_3__Time_to_clinical_improvement),
                         D4=as.character(new_data$ROB_4_Clinical_improvement),
                         D5=as.character(new_data$ROB_5_Time_to_clinical_improvement),
  ...
  interval=paste("[",format(round(analysis_data_09$LCI,digits = 2),nsmall = 2),", ",format(round(analysis_data_09$UCI,digits = 2),nsmall = 2),"]",sep="")
  analysis_data_09$ress=paste(format(round(analysis_data_09$HR,digits = 2),nsmall = 2),interval,sep=" ")
  analysis_data_09$ress=ifelse(analysis_data_09$ress=="NA [NA, NA]","",analysis_data_09$ress)
  ...
  if(keep_only=="Mild population"){
    data_sev<- analysis_data_09 %>%
      filter(analysis_data_09$sev==1)
    analysis_data_09=data_sev
  }
  
  if(keep_only=="Mixed population"){
    
    data_sev<-analysis_data_09 %>%
      filter(analysis_data_09$sev %in% c(2:11))
  ...
  if(keep_only=="Critical population"){
    ...
    data_sev<-analysis_data_09 %>%
      filter(analysis_data_09$sev==12)
  ...
  exclude_preprint<- analysis_data_09 %>%
    filter(analysis_data_09$Publication=="published paper")
  ...
  ################################################################
    ...
    w=setdiff(analysis_data_09$First_author,exclude_rob$First_author)
    
    w1=setdiff(analysis_data_09$First_author,exclude_rob1$First_author)
    
    if(high_RoB=="High RoB"){
      analysis_data_09<-subset(analysis_data_09,analysis_data_09$First_author %in% w)}
    
    
    if(high_RoB=="High RoB/Some concerns"){
      analysis_data_09<-subset(analysis_data_09,analysis_data_09$First_author %in% w1)}
  ...
  ilab=cbind(F_U_days=as.character(analysis_data_09$F_U_days),Intervention1=as.character(analysis_data_09$Intervention1),
             Intervention2=as.character(analysis_data_09$Intervention2),
             counts1=analysis_data_09$N1,counts2=analysis_data_09$N2,analysis_data_09$ress)
  ilab=as.data.frame(ilab)

  
  ########################################################################
  analysis_data_09_0<-subset(analysis_data_09,analysis_data_09$comp==comp.num & analysis_data_09$subgroup==0)
  analysis_data_09_1<-subset(analysis_data_09,analysis_data_09$comp==comp.num & analysis_data_09$subgroup==1)
  analysis_data_09_2<-subset(analysis_data_09,analysis_data_09$comp==comp.num & analysis_data_09$subgroup==2)
  ##########################################################################
  
  
  result_09=rma(measure="GEN",yi=analysis_data_09$logHR,sei=analysis_data_09$se,
                slab=paste(analysis_data_09$First_author,analysis_data_09$Year,sep=", "),
                subset=(analysis_data_09$comp==comp.num),method = het_estimate)
  
  
  
  
  result_091=metagen(TE=analysis_data_09$logHR,seTE=analysis_data_09$se,byvar = analysis_data_09$subgroup,method.tau = het_estimate)
  
  if(model=="Fixed-effects"){
    
    result_09=update(result_09,method = "FE")
  }
  
  pred_09<- predict(result_09, transf=exp, digits=2)
  pred_09 <- formatC(c(pred_09$pred, pred_09$ci.lb, pred_09$ci.ub), format="f", digits=2)
  
  
  ks0=0
  ks1=0
  ks2=0
  if (result_09$k>1 & nrow(analysis_data_09_0)>0){
    result_09.0=rma(measure="GEN",yi=analysis_data_09_0$logHR,sei=analysis_data_09_0$se,
                    slab=paste(analysis_data_09_0$First_author,analysis_data_09_0$Year,sep=", "),method = het_estimate)
    
    if(model=="Fixed-effects"){
      result_09.0=update(result_09.0,method = "FE")
      
    }
    
    ks0=result_09.0$k
    
    pred_09.0<- predict(result_09.0, transf=exp, digits=2)
    pred_09.0 <- formatC(c(pred_09.0$pred, pred_09.0$ci.lb, pred_09.0$ci.ub), format="f", digits=2)
    
  }
  if (result_09$k>1 & nrow(analysis_data_09_1)>0){
    result_09.1=rma(measure="GEN",yi=analysis_data_09_1$logHR,sei=analysis_data_09_1$se,
                    slab=paste(analysis_data_09_1$First_author,analysis_data_09_1$Year,sep=", "),method = het_estimate)
    
    if(model=="Fixed-effects"){
      result_09.1=update(result_09.1,method = "FE")
      
    }
    
    ks1=result_09.1$k
    
    pred_09.1<- predict(result_09.1, transf=exp, digits=2)
    pred_09.1<- formatC(c(pred_09.1$pred, pred_09.1$ci.lb, pred_09.1$ci.ub), format="f", digits=2)
    
  }
  if (result_09$k>1 & nrow(analysis_data_09_2)>0 ){
    result_09.2=rma(measure="GEN",yi=analysis_data_09_2$logHR,sei=analysis_data_09_2$se,
                    slab=paste(analysis_data_09_2$First_author,analysis_data_09_2$Year,sep=", "),method = het_estimate)
    
    if(model=="Fixed-effects"){
      
      result_09.2=update(result_09.2,method = "FE")
    }
    
    
    ks2=result_09.2$k
    
    pred_09.2<- predict(result_09.2, transf=exp, digits=2)
    pred_09.2<- formatC(c(pred_09.2$pred, pred_09.2$ci.lb, pred_09.2$ci.ub), format="f", digits=2)
  ...
  result_09.3=result_09
  
  data_rob=analysis_data_09 %>%
  ...
  analysis_data_09$var_hr=analysis_data_09$se*analysis_data_09$se
  analysis_data_09$weights=1/(analysis_data_09$var_hr+result_09$tau2)
  
  analysis_data_09$relative_weights=(analysis_data_09$weights/sum(analysis_data_09$weights,na.rm = T))*100
  
  analysis_data_09$relative_weights=format(round(analysis_data_09$relative_weights,digits = 2),nsmall=2)
  analysis_data_09$relative_weights=trimws(analysis_data_09$relative_weights)
  
  analysis_data_09$relative_weights=ifelse(analysis_data_09$relative_weights=="NA","",analysis_data_09$relative_weights)
  
  analysis_data_09$relative_weights=ifelse(analysis_data_09$relative_weights!="",
                                           paste(analysis_data_09$relative_weights,"%",sep = ""),analysis_data_09$relative_weights)
  
  ilab$weights=analysis_data_09$relative_weights
  ilab$weights=ifelse(ilab$weights!=" 0.00%",ilab$weights," ")
  
  forest=rob_append_to_forest(result_09,atransf=exp,showweights=F, xlab="Hazard Ratio",cex=0.8,mlab="",
  ...
  addpoly.rma(result_09.3,row=-1,mlab="",col="darkblue",border="darkblue",atransf=exp,cex=0.9,annotate = F)
  
  par(font=2)
  text(-5.1,-1,paste(pred_09[1], " [", pred_09[2], ",  ", pred_09[3], "]",sep = ""),cex = cex1)
  par(font=1)
  ...
  if (result_09$k>1){
    if (ks0>0 & ks1>0 & ks2==0){
      if(sub=="Severity"){
        addpoly.rma(result_09.1,row=1,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.0,row=ks1+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Conflicts of interest"){
        addpoly.rma(result_09.1,row=1,mlab="                    Studies with conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.0,row=ks1+2,mlab="                    Studies without conflicts of interest",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_09.1,row=1,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.0,row=ks1+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Location"){
        addpoly.rma(result_09.1,row=1,mlab="                    National studies",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.0,row=ks1+2,mlab="                    Multinational studies",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Separate Standard care/Placebo"){
        addpoly.rma(result_09.1,row=1,mlab="                    Placebo summary result",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.0,row=ks1+1.8,mlab="                    Standard care summary result",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        
      }
      
      
      par(font=2)
      text(-5.1,1,paste(pred_09.1[1], " [", pred_09.1[2], ",  ",pred_09.1[3], "]",sep = ""),cex = cex1)
      text(-5.1,ks1+1.8,paste(pred_09.0[1], " [", pred_09.0[2], ",  ", pred_09.0[3], "]",sep = ""),cex = cex1)
      par(font=1)
    ...
    if (ks0>0 & ks1==0 & ks2>0){
      if(sub=="Severity"){
        addpoly.rma(result_09.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.0,row=ks2+2,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      if(sub=="Funding"){
        addpoly.rma(result_09.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.0,row=ks2+2,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
    ...
    if (ks0==0 & ks1>0 & ks2>0){
      if(sub=="Severity"){
        addpoly.rma(result_09.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      if(sub=="Funding"){
        addpoly.rma(result_09.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      
      text(-5.1,1,paste(pred_09.2[1], " [", pred_09.2[2], ",  ", pred_09.2[3], "]",sep = ""),cex = cex1)
      text(-5.1,ks2+2,paste(pred_09.1[1], " [", pred_09.1[2], ",  ", pred_09.1[3], "]",sep = ""),cex = cex1)
    ...
    if (ks0>0 & ks1>0 & ks2>0){
      if(sub=="Severity"){
        addpoly.rma(result_09.2,row=1,mlab="                    Critical population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.1,row=ks2+2,mlab="                    Mixed population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.0,row=ks2+4+ks1,mlab="                    Mild population",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      }
      if(sub=="Funding"){
        addpoly.rma(result_09.2,row=1,mlab="                    Studies without specific funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.1,row=ks2+2,mlab="                    Studies with public/non profit funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
        addpoly.rma(result_09.0,row=ks2+4+ks1,mlab="                    Studies with mixed/private funding",col="blue",border="blue",atransf=exp,cex=0.9,annotate = F)
      ...
      text(-5.1,1,paste(pred_09.2[1], " [", pred_09.2[2], ",  ", pred_09.2[3], "]",sep = ""),cex = cex1)
      text(-5.1,ks2+2,paste(pred_09.1[1], " [", pred_09.1[2], ",  ", pred_09.1[3], "]",sep = ""),cex = cex1)
      text(-5.1,ks2+4,paste(pred_09.0[1], " [", pred_09.0[2], ",  ", pred_09.0[3], "]",sep = ""),cex = cex1)
  ...
  text(-4.5,3.5+max(allrows),"Time to clinical improvement",
       pos=4,
       cex=1)
  ...
  if(hide_dose=="FALSE"){
    if(comp.num==comp.num){
      if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
        for(i in 1:result_09$k){
          text(-11.5, -0.3+i+sliderDose, pos=4, analysis_data_09$dose[i],cex=cex1-0.3)
        }
      }
      if (ks0>0 & ks1>0 & ks2==0){
        for(i in 1:ks1){
          text(-11.5, -0.3+i+1+sliderDose, pos=4, analysis_data_09$dose[i],cex=cex1-0.3)
        }
        for(i in (ks1+1):(ks0+ks1)){
          text(-11.5, -0.3+i+2+sliderDose, pos=4, analysis_data_09$dose[i],cex=cex1-0.3)
        }
      }
      if (ks0>0 & ks1==0 & ks2>0){
        for(i in 1:ks2){
          text(-11.5, -0.3+i+1+sliderDose, pos=4, analysis_data_09$dose[i],cex=cex1-0.2)
        }
        for(i in (ks2+1):(ks0+ks2)){
          text(-11.5, -0.3+i+2+sliderDose, pos=4, analysis_data_09$dose[i],cex=cex1-0.2)
        }  
      }
      if (ks0==0 & ks1>0 & ks2>0){
        for(i in 1:ks2){
      ...
      if (ks0>0 & ks1>0 & ks2>0){
        ...
        for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
          text(-11.5, -0.3+i+4+sliderDose, pos=4, analysis_data_09$dose[i],cex=cex1-0.2)
        }
      }
    }
    
    
    count=sum(str_count(analysis_data_09$dose, "\\*"),na.rm = TRUE)
  ...
  if ((ks0==0 & ks2==0) | (ks0==0 & ks1==0) | (ks1==0 & ks2==0)){
    for(i in 1:result_09$k){
      if(hide_severity=="FALSE"){
        text(-15, 0.5+i, pos=4, analysis_data_09$severity[i],cex=cex1-0.1)
      }
    }
  }
  if (ks0>0 & ks1>0 & ks2==0){
    for(i in 1:ks1){
      if(hide_severity=="FALSE"){
        text(-15, 0.5+i+1, pos=4, analysis_data_09$severity[i],cex=cex1-0.1)
      }
    }
    for(i in (ks1+1):(ks0+ks1)){
      if(hide_severity=="FALSE"){
        text(-15, 0.5+i+2, pos=4, analysis_data_09$severity[i],cex=cex1-0.1)
      }
    }
  }
  if (ks0>0 & ks1==0 & ks2>0){
  ...
  if (ks0>0 & ks1>0 & ks2>0){
    ...
    for(i in (ks1+ks2+1):(ks1+ks2+ks0)){
      if(hide_severity=="FALSE"){
        text(-15, 0.5+i+4, pos=4, analysis_data_09$severity[i],cex=cex1-0.1)
      }
    }
  }
  ...
  if(nrow(analysis_data_09)>1){
    ...
    N1_total=sum(analysis_data_09$N1)
    N2_total=sum(analysis_data_09$N2)
  ...
  text(-2.5,-1.5,"Intervention 2 better",
       pos=4,
       cex=cex1-0.1)
  text(0.5,-1.5,"Intervention 1 better",
       pos=4,
       cex=cex1-0.1)
  ...
  if(result_09$k>8.5){
    if(length(unique(analysis_data_09$subgroup))==1){
      par(font=2)
      text(-15,-1.5 , pos=4, cex=cex1, bquote(paste("Heterogeneity results: Q = ",
                                                    .(formatC(result_09$QE, digits=2, format="f")),
                                                    ", p = ", .(formatC(result_09$QEp, digits=2, format="f")), "; ", I^2, " = ",
                                                    .(formatC(result_09$I2, digits=1, format="f")), "%; ",tau^2," = ",
                                                    .(formatC(result_09$tau2, digits=2, format="f")))))  
    }
    if(length(unique(analysis_data_09$subgroup))>1){
      par(font=2)
      ...
      text(-15, -2.5, pos=4, cex=cex1, bquote(paste("Test for subgroup differences: Q = ",
                                                    .(formatC(result_091$Q.b.random, digits=2, format="f")),"; ", "p = ",
                                                    .(formatC(result_091$pval.Q.b.random, digits=2, format="f"))
  ...
  if((result_09$k>1) & (result_09$k<9)){
    if(length(unique(analysis_data_09$subgroup))==1){
      par(font=2)
      text(-15,-1.5 , pos=4, cex=cex1, bquote(paste("Heterogeneity results: Q = ",
                                                    .(formatC(result_09$QE, digits=2, format="f")),

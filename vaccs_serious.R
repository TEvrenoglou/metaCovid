...
)
{
  ...
  data$F_U_days=ifelse(is.na(data$SAE_TP),data$F_U_months,as.numeric(data$SAE_TP))
  ...
  data$Developer_name=ifelse(data$Developer_name=="AstraZeneca+University of Oxford",
                             "University of Oxford+AstraZeneca/Serum Institute of India",data$Developer_name)
  
  data$Developer_name=ifelse(data$Developer_name=="Serum Institute of India/AstraZeneca+University of Oxford",
                             "University of Oxford+AstraZeneca/Serum Institute of India",data$Developer_name)
  ...
  data$n_Randomized=as.numeric(data$SAE_denominator)
  ...
  data$Serious_AE_D1428=data$Serious_AE
  
  
  rct_data=data
  
  
  rct_data$Serious_AE_D1428=as.numeric(rct_data$Serious_AE_D1428)
  pair_data_05_14=pairwise_old(studlab=Trial_ID,treat=Treat_Name,event=Serious_AE_D1428,n=n_Randomized,
...
analysis_data_05_14=cbind.data.frame(Trial_ID=new_data_05_14$Trial_ID,First_author=new_data_05_14$First_author,
                                     ...
                                     B=as.character(new_data_05_14$ROB2_Serious_adverse_events),
                                     C=as.character(new_data_05_14$ROB_3_Serious_adverse_events),
                                     D=as.character(new_data_05_14$ROB_4_Serious_adverse_events),
                                     E=as.character(new_data_05_14$ROB_5_Serious_adverse_events),
                                     Overall=as.character(new_data_05_14$ROB_6_Serious_adverse_events),
...
rob=setdiff(analysis_data_05_14$First_author,exclude_rob$First_author)

rob1=setdiff(analysis_data_05_14$First_author,exclude_rob1$First_author)

if(high_RoB=="High RoB"){
  analysis_data_05_14<-subset(analysis_data_05_14,analysis_data_05_14$First_author %in% rob)}


if(high_RoB=="High RoB/Some concerns"){
  analysis_data_05_14<-subset(analysis_data_05_14,analysis_data_05_14$First_author %in% rob1)}
...
slab=paste(analysis_data_05_14$First_author,year_comp,sep=", ")

p1=escalc(ai=analysis_data_05_14$r1,bi=analysis_data_05_14$f1,ci=analysis_data_05_14$r2,
          di=analysis_data_05_14$f2,slab=slab,
...
if(model!="Fixed-effects"){
  meta=metagen(TE=yi,seTE=sqrt(vi),subgroup =   Developer_name,data = analysis_data_05_14,method.tau = het_estimate)
}else{
  meta=metagen(TE=yi,seTE=sqrt(vi),subgroup = Developer_name,data = analysis_data_05_14,comb.fixed = T,comb.random = F)
}
...
forest=rob_append_to_forest(res,atransf=exp,
                            ...
                            at=log(c(0.1,1,5)),
                            steps=4,
                            ilab=ilab,ilab.xpos=c(-12.35,-11.2,-9.3,-8,-7,#-6.1,
...
if(subgroup==T){
header=c("Follow up\nmonths","Intervention 1","Intervention 2","r1/N1","r2/N2","Weights","RR [95% CI]")
}else{
  header=c("Follow up\nmonths","Intervention 1","Intervention 2","r1/N1","r2/N2","RR [95% CI]")  
}
...
par(font=1)
text(-2.5,3.5+max(rows),"Serious adverse events",
     pos=4,
     cex=1)
# 
...
for(i in 1:max(u)){
  ...
  if(length(yy[[i]])>1 & events.dev[[i]]!=0){
    ...
    rel_weights[[i]]=weights[[i]]/sum(weights[[i]])*100
...
if(type=="Non replicating viral vector"){
  text(-15, -0.9, "Sadoff J, 2021b: Results from the FDA briefing report", pos=4, cex=0.7)
  text(-15, -1.5, "Madhi S, 2021: Participants living with HIV", pos=4, cex=0.7)
  text(-15, -2.1, "MenACWY: Quadrivalent meningococcal conjugate vaccine", pos=4, cex=0.7)
}

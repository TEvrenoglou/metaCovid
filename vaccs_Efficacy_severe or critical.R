forest_severe=function(data,type,subgroup=T,model="Random-effects",het_estimate="REML",
                       high_RoB=FALSE,Publication=FALSE) {
  
  data=subset(data,
              Trial_ID!=66 & Trial_ID!=5015 & Trial_ID!= 47 & Trial_ID!= 67 & Trial_ID!= 73 & Trial_ID!= 75)
  ...
  data$F_U_months=ifelse(is.na(data$Conf_severe_or_critical_TP),as.character(data$Efficacy_median_followup),data$Conf_severe_or_critical_TP)
  data$F_U_days=data$F_U_months
  data$Start_F_U_days=as.character(data$Conf_sympt_time_zero)
  data$n_Randomized=as.numeric(data$severe_critical_denominator)
  ...
  data$status=ifelse(is.na(data$Vaccine_efficacy_severity_LCI),"Incomplete data",data$status)
  
  data$status=ifelse(is.na(data$Vaccine_efficacy_severity_UCI),"Incomplete data",data$status)
  ...
  rct_data$Conf_severe_or_critical=as.numeric(rct_data$Conf_severe_or_critical)
  pair_data=pairwise_old(studlab=Trial_ID,treat=Treat_Name,event=Conf_severe_or_critical,n=severe_critical_denominator,
  ...
  pair_data$Conf_sympt1=as.numeric(pair_data$Vaccine_efficacy_severity)
  pair_data$Conf_sympt_CI_Lower1=as.numeric(pair_data$Vaccine_efficacy_severity_LCI)
  pair_data$Conf_sympt_CI_Upper1=as.numeric(pair_data$Vaccine_efficacy_severity_UCI)
  ...
  new_data=pair_data1 %>%
    ...
    new_data$Start_F_U_days=new_data$Start_F_U_days
    
 
  
  analysis_data_09=cbind.data.frame(Trial_ID=new_data$Trial_ID,First_author=new_data$First_author,
                                    F_U_days=new_data$F_U_days,F_U_months=new_data$F_U_months1,Start=new_data$Start_F_U_days,
                                    ...
                                    percentage=new_data$Conf_sympt1,LCI=new_data$Vaccine_efficacy_severity_LCI,
                                    UCI=new_data$Vaccine_efficacy_severity_UCI,
                                    ...
                                    B=as.character(new_data$ROB2_Severe_or_critical),
                                    C=as.character(new_data$ROB_3_Severeorcritical),
                                    D=as.character(new_data$ROB_4_Severeorcritical),
                                    E=as.character(new_data$ROB_5_Severeorcritical),
                                    Overall=as.character(new_data$ROB_6_Severeorcritical),
  ...
  ilab=cbind(Start=analysis_data_09$Start,F_U_days=as.character(analysis_data_09$F_U_months),Intervention1=as.character(analysis_data_09$Intervention1),
  ...
  text(-3.3,3.5+max(rows),"Severe or critical COVID-19 after complete vaccination",
       pos=4,
       cex=1)
  ...
  if(type=="Inactivated virus"){
    ...
    text(-3, -0.6, "Tanriover M, 2021: Hospitalised (moderate or severe)", pos=4, cex=0.75)
  }
